  ...
  try {
    ...
    const schemaRes = await fetch(apiUrl, {
      ...
      body: JSON.stringify({
        ...
        arguments: { 
          query: "SELECT sql FROM sqlite_master WHERE name = 'user'" 
        }
      })
    }).then(res => res.json());
    console.log('USER_SCHEMA:', schemaRes.result.content[0].text);
    ...
    fs.writeFileSync('user_backup.json', JSON.stringify(rows, null, 2));
    console.log('Saved to user_backup.json');
