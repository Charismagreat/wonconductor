...
export async function getVisualizationRecommendation(
  ...
  const systemPrompt = `
    ...
    6. **마이너스 통장 및 한도 거래 계좌 해석 (CRITICAL)**:
       - '현재잔액'이 음수(-)이더라도 '약정금액(한도)'과 '사용가능한도'가 제공되는 계좌는 회사의 실질적인 '가용 자금'으로 분류됩니다.
       - 대시보드나 금융 보고서 작성 시 이를 단순히 마이너스 부채로 보지 말고, 실제 인출 가능한 가용 자금 금액(사용가능한도)을 기준으로 똑똑하게 분석하고 사용자에게 설명하십시오. (예: "KB기업종합통장은 마이너스 한도 거래가 설정되어 있어, 현재 잔액은 -5,441만 원이지만 1억 원의 지급한도가 있으므로 실제 사용 가능한 가용 자금은 4,558만 원입니다.")
    7. **보안 지침**: SQL을 직접 작성할 수 없으므로, 모든 데이터 조작은 제공된 도구의 인자를 통해서만 수행하십시오.
