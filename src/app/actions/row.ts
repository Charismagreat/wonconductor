 ...
 */
const unwrap = (res: any) => Array.isArray(res) ? res : (res?.rows || []);
...
export async function addRowAction(reportId: string, rowData: any) {
  ...
  // ID 정규화: 항상 rep- 형식이거나 마스터에 정의된 고유 ID 사용 (dashboard_data와 일치시키기 위함)
  const canonicalReportId = report.reportId || String(report.id);

  const columns = JSON.parse(report.columns);
  
  // 1. 유효성 검사 및 데이터 정제
  let { isValid, error, cleanedData } = ValidationService.validateRowData(rowData, columns);
  if (!isValid) throw new Error(error);

  // 추가 정제: 컬럼 정의에 없는 필드는 제거 (수동 입력 폼에서 유입되는 불필요한 필드 차단)
  const sanitizedData: any = {};
  const columnNames = new Set(columns.map((c: any) => c.name));
  for (const key of Object.keys(cleanedData)) {
      if (columnNames.has(key) || key.startsWith('__')) {
          sanitizedData[key] = cleanedData[key];
      }
  }
  ...
  try {
      const existingRows = unwrap(await queryTable('dashboard_data', {
          filters: { reportId: canonicalReportId, contentHash: String(hash), __is_deleted: '0' }
      }));
  ...
  // 3. 자동 ID 생성 (전역 표준: DID- + 7자리 숫자)
  const idCol = columns.find((c: any) => c.isAutoGenerated);
  if (idCol) {
      const nextSerial = await RowService.calculateNextSerial();
      cleanedData[idCol.name] = `DID-${nextSerial.toString().padStart(7, '0')}`;
      // 해당 보고서의 lastSerial을 전역 최신값으로 업데이트하여 다음 번호 생성에 반영
      await updateRows('dashboard_master', { lastSerial: nextSerial }, { filters: { reportId: canonicalReportId } }).catch(console.error);
  }
  
  // 시스템 컬럼 데이터 할당 (표준 규격 - KST)
  const now = getNowKST();
  ...
  // 4. 저장 (물리 + 가상 테이블)
  const saveResult: any = await RowService.saveRow(canonicalReportId, report.tableName, columns, cleanedData, hash, user.id);
...
export async function addRowsAction(reportId: string, rows: any[]) {
    ...
    const existingRows = unwrap(await queryTable('dashboard_data', { filters: { reportId: String(reportId), __is_deleted: '0' } }));
...
export async function deleteRowsAction(reportId: string, rowIds: string[]) {
    ...
    for (const id of rowIds) {
        const rows = unwrap(await queryTable('dashboard_data', { filters: { id: String(id) } }));
        const row = rows[0];
        if (!row) continue;
    ...
    logActivity({
        ...
        message: `${deletedCount}건의 데이터를 삭제했습니다.`,
        link: `/report/${reportId}`,
        type: 'ACTIVITY'
    }).catch(console.error);
...
export async function restoreRowsAction(reportId: string, rowIds: string[]) {
    ...
    for (const id of rowIds) {
        ...
        const existings = unwrap(await queryTable('dashboard_data', { 
...
export async function checkSyncStatusAction(reportId: string) {
    try {
        ...
        const virtualResult = unwrap(await queryTable('dashboard_data', { 
            filters: { reportId: String(reportId), __is_deleted: '0' } 
        }));
        const virtualCount = virtualResult.length;

        const physicalResult = unwrap(await queryTable(report.tableName, { 
            filters: { __is_deleted: '0' },
            limit: 100000 
        }));
        const physicalCount = physicalResult.length;
