...
| `[x]` [계좌 거래 디버깅 재조사 1] 사용자가 조회하는 위젯 및 캐시/새로고침 상태 분석 | 완료 | 위젯 ID 9(최근 7일간 거래내역)의 config.data 캐시에 306-063568-04-036의 실시간 거래들이 정상 적재되었음을 확인 완료 |
| `[x]` [계좌 거래 디버깅 재조사 2] 위젯의 강제 새로고침(Refresh) 검증 | 완료 | 데이터가 DB 캐시에 완전히 써져 있으며, SWR로 인해 화면 갱신을 위한 브라우저 강력 새로고침이 필요함을 식별 완료 |
| `[x]` [계좌 거래 디버깅 재조사 3] 정적 빌드 및 동작 최종 검증 | 완료 | 최신 대출 계좌 거래 내역이 통합 위젯 캐시에 완전히 갱신 완료되어 빌드 무결성 유지 상태임을 검증 완료 |
| `[x]` [잔액 현황표 디버깅 Phase 1] "모든 계좌 잔액 현황표" 위젯 식별 및 소스 분석 | 완료 | 해당 위젯의 DB ID와 쿼리 헬퍼, 로직 구조 파악 완료 |
| `[x]` [잔액 현황표 디버깅 Phase 2] 계좌 306-063568-04-036의 누락 원인 규명 | 완료 | 마이너스 통장임에도 대출 계좌로 필터링되어 소거된 것이 원인임을 규명 완료 |
| `[x]` [잔액 현황표 디버깅 Phase 3] 가설 설정 및 최소 수정안 도출 | 완료 | 대출 계좌 소거 로직에서 마이너스 통장은 배제하는 조건 설계 완료 |
| `[x]` [잔액 현황표 디버깅 Phase 4] 코드 수정 및 정적 빌드/동작 검증 | 완료 | 수정 적용 및 Next.js 빌드 성공, 마이너스 통장은 노출하고 대출 계좌는 건너뛰도록 처리 |
