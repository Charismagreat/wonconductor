// test_hana_loan_latest.ts
import { queryBankProductTable } from './egdesk-helpers';

async function testHanaLoanLatest() {
  const tableSlug = 'hana_loan_history';
  ...
  try {
    ...
    if (normalRes.rows && normalRes.rows.length > 0) {
      ...
      normalRes.rows.slice(0, 5).forEach((row: any, i: number) => {
        console.log(`  [${i + 1}] ID: ${row.id}, 계좌번호: ${row.account_number}, 일자: ${row.transaction_date || row.date}, 금액: ${row.amount || row.transaction_amount}, 잔액: ${row.balance || row.loan_balance}`);
      });
    }
    ...
    if (normalRes.rows && latestRes.rows) {
      console.log('\n--- 3. 검증 결과 분석 ---');
      const normalAccs = normalRes.rows.map((r: any) => r.account_number);
      const latestAccs = latestRes.rows.map((r: any) => r.account_number);
      
      const hasDuplicatesInNormal = normalAccs.length !== new Set(normalAccs).size;
      const hasDuplicatesInLatest = latestAccs.length !== new Set(latestAccs).size;
      
      console.log(`일반 쿼리 내 중복 계좌번호 존재 여부: ${hasDuplicatesInNormal ? '있음 (중복 존재)' : '없음 (고유)'}`);
      console.log(`최신 쿼리(latestOnly) 내 중복 계좌번호 존재 여부: ${hasDuplicatesInLatest ? '있음' : '없음 (정상 필터링됨)'}`);
      
      if (hasDuplicatesInNormal && !hasDuplicatesInLatest) {
        console.log('성공: latestOnly 옵션이 동일한 계좌번호에 대해 가장 최신 데이터만 정상적으로 필터링하여 반환하고 있습니다!');
      } else {
        console.log('안내: 이 테스트 데이터셋에는 원래 계좌번호 중복이 없거나, 또는 다른 방식으로 동작하고 있습니다.');
      }
    }
