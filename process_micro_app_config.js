  ...
  try {
    const schemaRes = await fetch(apiUrl, {
      ...
      body: JSON.stringify({
        ...
        arguments: { 
          query: "SELECT sql FROM sqlite_master WHERE name = 'micro_app_config'" 
        }
      })
    }).then(res => res.json());
    ...
    const dataRes = await fetch(apiUrl, {
      ...
      body: JSON.stringify({
        tool: 'user_data_sql_query',
        arguments: { 
          query: "SELECT * FROM micro_app_config" 
        }
      })
    }).then(res => res.json());
    ...
    fs.writeFileSync('micro_app_config_backup.json', JSON.stringify(rows, null, 2));
    console.log('Saved to micro_app_config_backup.json');
