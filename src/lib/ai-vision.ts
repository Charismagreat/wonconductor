...
export async function extractDataFromImage(imageBase64: string, mimeType: string, columns: any[], aiRulesPrompt: string = ''): Promise<any> {
  ...
  const prompt = `
    ...
    분석 및 응답 규칙:
    1. 사진 성격 파악: 명함, 영수증, 견적서, 리스트 등 이미지의 성격을 파악하고 그에 맞는 정보를 추출하세요.
    2. 거래처 식별 (가장 중요): 
       - 문서 내에 사업자 정보 박스(공급자란 등)가 있다면, 거기서 **'사업자등록번호'**와 **'상호(회사명)'**를 최우선으로 추출하세요.
       - 수신인이 '원컨덕터' 또는 '애월삼춘'인 경우, 이는 매입 서류이므로 **'공급자(발신인)'**의 정보를 추출해야 합니다.
       - 거래처ID 매칭의 정확성을 위해 '사업자등록번호'와 '상호' 최소 2가지 정보를 반드시 함께 확보하도록 노력하세요.
    3. 다중 데이터 대응: 이미지 내에 여러 개의 데이터(예: 견적서의 여러 품목 줄, 명단 리스트의 여러 사람 등)가 존재할 경우, 각 데이터를 독립된 객체로 구성하여 반드시 **JSON 배열 ([{...}, {...}])** 형식으로 응답하세요. 데이터가 하나뿐이라면 단일 객체({...})로 응답해도 무방합니다.
    4. 평탄화(Flattening): 문서의 공통 정보(날짜, 거래처명/사업자번호 등)와 상세 품목(품목명, 수량 등)이 함께 있는 경우, 상세 품목의 각 줄마다 공통 정보를 중복해서 포함시켜 '단일 행'으로 구성하세요.
    5. 반드시 유효한 JSON 형식으로만 응답하세요. (다른 설명 텍스트 금지)
    6. 필드 이름은 위 리스트에 제공된 '이름'과 정확히 일치해야 합니다.
    7. 날짜(date) 유형은 'YYYY-MM-DD' 형식을 엄격히 지켜야 합니다. (이미지에 연도가 없으면 현재 연도 2026 사용)
    8. 숫자(number) 또는 통화(currency) 유형은 쉼표, 원화 기호, 공백 없이 숫자만 반환하세요.
    9. 목록 선택(select) 유형은 상기 제공된 '옵션' 중 이미지 내용과 가장 잘 맞는 하나를 정확히 선택하세요.
    10. 정보를 도저히 찾을 수 없는 경우 해당 필드 값에 null을 넣으세요.
    ${aiRulesPrompt}
  `;
