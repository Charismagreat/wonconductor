...
async function restore() {
  console.log('Restoring notification data without IDs...');
  try {
    const rawData = fs.readFileSync('notification_backup.json', 'utf8');
    ...
    const checkRes = await fetch(apiUrl, {
      ...
      body: JSON.stringify({
        ...
        arguments: { 
          query: "SELECT id, userId, title FROM notification" 
        }
      })
    }).then(res => res.json());
